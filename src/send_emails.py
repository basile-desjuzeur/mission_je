...
DATASET_FILE = "../data/241003_cleaned_dataset.csv"
MAIL_OBJECT = (
...
def emailing_and_clean_by_chunk(
    mail_file=MAIL_FILE, dataset_file=DATASET_FILE, chunksize=500
):
    """
    ...
    for i in range(len(data)):
        ...
        send_email_with_html(
            email_recipient, MAIL_OBJECT, mail_file, name, random_time_spacing=True
        )
...
def main():
    ...
    while pd.read_csv(DATASET_FILE)["SENT"].sum() < pd.read_csv(DATASET_FILE).shape[0]:
